...
export default function CashierPayment() {
  ...
  const fetchBankDetails = (phoneNumber) => {
    fetch(`http://172.31.7.181:8080/getUserBankDetails?phoneNumber=${phoneNumber}`, {
      method: 'GET',
      headers: {
  ...
  const fetchBtcQty = (phoneNumber) => {
    fetch(`http://172.31.7.181:8080/getAvailableBtcQtyForPhoneNumber?phoneNumber=${phoneNumber}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('authToken')}`
      }
  ...
  const handleSubmit = () => {
    if (isValid) {
      fetch('http://172.31.7.181:8080/cashierPayment', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
